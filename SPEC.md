...
| réseau | le **pare-feu du conteneur** (4.1 bis) : règles posées par l'entrypoint en root avant l'abandon des privilèges, échec = conteneur qui refuse de démarrer, DNS restreint au résolveur du moteur, aucune plage privée ouverte, liste blanche par rôle | un domaine derrière un CDN change d'adresse ; le pare-feu résout au démarrage et `hq check` sonde de l'intérieur |
